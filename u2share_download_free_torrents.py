        ...
        if '<b>有效</b>' in v:
            if '全局' not in v and username not in v:  # 排除全局魔法 自己创建的魔法
                ...
                if 'alt="Promotion"' in v:  # 自定义魔法
                    ...
                    log.info(f'{Magic_UP}↑ {Magic_DO}↓ ==> 魔法的ID:{Magic_ID}，种子的ID：{Torrent_ID}，魔法结束时间：{Magic_Start_Time}，魔法的时长：{Magic_Time}')
                ...
                if 'alt="FREE"' in v:  # 1x 上传 0x 下载
                    ...
                    Torrent_ID_List.append(Torrent_ID)
                    log.info(f'1.00X↑ 0.00X↓ ==> 魔法的ID:{Magic_ID}，种子的ID：{Torrent_ID}，魔法结束时间：{Magic_Start_Time}，魔法的时长：{Magic_Time}')
                    ...
                    Magic_Time = (re.findall(re3, v))[0][1]
                    Torrent_ID_List.append(Torrent_ID)
                    log.info(f'0.00X↑ 0.00X↓ ==> 魔法的ID:{Magic_ID}，种子的ID：{Torrent_ID}，魔法结束时间：{Magic_Start_Time}，魔法的时长：{Magic_Time}')
