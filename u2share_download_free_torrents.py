...
def AnalysisMagic(html):
    ...
    twoupfree_bg = soup.find_all('tr', attrs={'class': 'twoupfree_bg'})  # 2x 上传 0x 下载
    Magic(twoupfree_bg)
    free_bg = soup.find_all('tr', attrs={'class': 'free_bg'})  # 1x 上传 0x 下载
    Magic(free_bg)
    custompromotion_bg = soup.find_all('tr', attrs={'class': 'custompromotion_bg'})  # ?x 上传 ?x 下载
        ...
        if '<b>有效</b>' in v:
            if '全局' not in v and username not in v:  # 排除全局魔法 自己创建的魔法
                ...
                if 'alt="Promotion"' in v:  # 自定义魔法
                    ...
                    log.logger.info(f'{Magic_UP}↑ {Magic_DO}↓ ==> 魔法的ID:{Magic_ID}，种子的ID：{Torrent_ID}，魔法结束时间：{Magic_Start_Time}，魔法的时长：{Magic_Time}')
                ...
                if 'alt="FREE"' in v:  # 1x 上传 0x 下载
                    ...
                    Torrent_ID_List.append(Torrent_ID)
                    log.logger.info(f'1.00X↑ 0.00X↓ ==> 魔法的ID:{Magic_ID}，种子的ID：{Torrent_ID}，魔法结束时间：{Magic_Start_Time}，魔法的时长：{Magic_Time}')
                    ...
                    Magic_Time = (re.findall(re3, v))[0][1]
                    Torrent_ID_List.append(Torrent_ID)
                    log.logger.info(f'0.00X↑ 0.00X↓ ==> 魔法的ID:{Magic_ID}，种子的ID：{Torrent_ID}，魔法结束时间：{Magic_Start_Time}，魔法的时长：{Magic_Time}')
...
def Savecsv(_id):
    with open(f'{abs_path}/FreeIdList.csv', 'a', newline='', encoding='utf-8-sig') as csv_file:
...
if __name__ == "__main__":
    ...
    AnalysisMagic(OpenHtml('https://u2.dmhy.org/promotion.php?action=list&page=0'))
    AnalysisMagic(OpenHtml('https://u2.dmhy.org/promotion.php?action=list&page=1'))
    # AnalysisMagic(OpenHtml('https://u2.dmhy.org/promotion.php?action=list&page=2'))

    if os.path.exists(f'{abs_path}/FreeIdList.csv'):
        with open(f'{abs_path}/FreeIdList.csv', "r", encoding='utf-8-sig') as csv_file:
    ...
    for v in Torrent_ID_List:
        ...
        if str(torrent_id) in _IdList:
            ...
            log.logger.info(f'{save_path}')
            if QBT.AddTorrent(QBT.Login(), torrent_id, f'{torrent_id}', save_path) == 200:
                log.logger.info('种子成功添加！开始下载...')
                time.sleep(25)
                _hash = QBT.GetHash(QBT.Login(), f'{torrent_id}')
                if QBT.SetForceStart(QBT.Login(), _hash) == 200:
                    Savecsv(torrent_id) # 写入数据库，防止重复下载
                    log.logger.info('启动下载成功！')
                else:
                    log.logger.debug('启动下载失败！')
                    time.sleep(1)
